...
`DecisionOps Control Tower` pipeline과 FastAPI service가 Stage 1/2 산출물을 읽어 control state, review queue, API contract, dashboard, approval history로 변환하는 계약을 기록한다.
...
원천 간 raw row join은 하지 않는다. Stage 1/2 산출물을 run-level control state로 결합하고, Stage 2 review queue row를 Control Tower approval queue row로 projection한다. Seoul Ddareungi priority row는 `impact_cards`로 projection하며, validation summary가 `READY`가 아니면 `guardrail_state=validation_not_ready`와 public-claim blocker를 붙인다.
...
- Impact card의 후보 단위는 검증 전 production 성과가 아니라 reviewer evidence이며, Seoul validation이 `READY` 전이면 verified improvement를 비워 둔다.
